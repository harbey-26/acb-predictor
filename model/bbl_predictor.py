...
def predict(equipo_local: str, equipo_visitante: str) -> Dict:
    ...
    return {
        ...
        "confianza": confianza,
        "features_usadas": {
            "home_win_rate_reciente": float(round(s(hs5, "win_rate"), 3)),
            "away_win_rate_reciente": float(round(s(as5, "win_rate"), 3)),
            "home_pts_promedio": float(round(s(hs5, "pts_avg"), 1)),
            "away_pts_promedio": float(round(s(as5, "pts_avg"), 1)),
            "home_val_promedio": float(round(s(hs5, "pt_diff"), 1)),
            "away_val_promedio": float(round(s(as5, "pt_diff"), 1)),
